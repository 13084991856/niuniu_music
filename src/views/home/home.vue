      ...
      <layout class="layoutBox">
        <el-carousel :interval="4000" type="card" height="245px">
          <el-carousel-item v-for="(item, index) in latestAlbum" :key="index">
            <a :href="item.url">
              <img
                :src="item.imageUrl + '?param=661y245'"
                alt=""
                class="img-row-2"
              />
            </a>
      ...
      <layout>
        ...
        <ul class="RecommendSong">
          <li
            ...
            @mouseover="showdiv(index + 1)"
            @click="playSong(item.id)"
          >
            <div style="display: inline-block; padding-left: 25px">
              <a
                href="javascript:;"
                class="el-icon-video-play hide"
                :class="{ showIcon: isshowturediv == index + 1 }"
              ></a>
              <span
                class="marginbox"
                :class="{ hide: isshowturediv == index + 1 }"
                >0{{ index + 1 }}</span
              >
            </div>
            <!-- 歌曲封面 -->
            ...
            <!-- 歌曲时长 -->
            <div class="time">
              <div>{{ item.song.bMusic.playTime | GetTime() }}</div>
            </div>
          </li>
        </ul>
      </layout>
  ...
  methods: {
    ...
    GetLatestSong() {
      GetLatestSong().then((res) => {
        ...
        // this.latestsong.forEach(val=>{
        //   console.log(val.song.bMusic.playTime,val.song.mMusic.playTime);
        // })
      });
    },
