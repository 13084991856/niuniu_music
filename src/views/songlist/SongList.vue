  ...
  methods: {
    ...
    FilterData(arr) {
      ...
      for (let i = 0; i < Object.keys(this.categories).length; i++) {
        array.push(arr.filter((item) => item.category == i))
      }
      // let res0 = arr.filter((item) => item.category == 0);
      // let res1 = arr.filter((item) => item.category == 1);
      // let res2 = arr.filter((item) => item.category == 2);
      // let res3 = arr.filter((item) => item.category == 3);
      // let res4 = arr.filter((item) => item.category == 4);
    ...
    GetPlaylistContent(order, cat, limit, offset) {
      GetPlaylistContent(order, cat, limit, offset).then((res) => {
        ...
        this.total = Math.ceil(res.total / this.limit) * 10;
      });
    },
    // 获取 全部歌单分类 -> 然后分类
    GetAllPlaylist() {
