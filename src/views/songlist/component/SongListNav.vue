<template>
  <div class="SongListNav">
    <ul>
      <li class="all" @click="show = !show">
        全部 <i class="My-new-iconbianji"></i>
      </li>
      ...
      <li class="LastListHot">
        <span
          class="hot"
          :class="{ activeBackgroundColor: 1 == currentRightItim }"
          @click="ChangerRightColor(1)"
          ><i class="My-new-iconhuore"></i>热门</span
        >
      </li>
      <li class="LastListNew">
        <span
          class="new"
          :class="{ activeBackgroundColor: 2 == currentRightItim }"
          @click="ChangerRightColor(2)"
          ><i class="My-new-iconshuaxin"></i>最新</span
        >
      </li>
    </ul>
        ...
        <div v-show="show" class="transition-box">
          <!-- 
              两层循环嵌套  外层循环是 ArrayData 就是 一级分类 比如语言、情感
                            里层循环是 ArrayData的item项  就是 一级分类下的二级分类 比如语言里面的 类别
             -->
          <div
            class="transition-box-box"
            :key="index"
