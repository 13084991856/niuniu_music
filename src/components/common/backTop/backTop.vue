    ...
    backTop(e) {
      ...
      let animation = setInterval(() => {
        ...
        if (start > end) {
          ...
          // 回退起点
          e.target.setAttribute("style", " bottom:100px; display: none;");
          // this.flag = false;
          // 播放800毫秒的特效
          setTimeout(() => {
            this.flagBack = false;
  ...
  mounted() {
    window.addEventListener(
      ...
      () => {
        ...
        if (
          (document.body.scrollTop >= 102 ||
            window.pageYOffset >= 102 ||
            document.documentElement.scrollTop >= 102)&&nav
        ) {
          this.flag = true;
        } else {
          this.flag = false;
